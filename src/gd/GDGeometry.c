...
void GDSetVtxDescv(const GXVtxDescList* attrPtr)
{
  ...
  for (; attrPtr->attr != 0xff; ++attrPtr) {
    ASSERTMSGLINE(88,
      attrPtr->attr >= GX_VA_PNMTXIDX && attrPtr->attr <= GX_VA_MAX_ATTR,
      "GDSetVtxDescv: invalid attribute");
    ASSERTMSGLINE(92, attrPtr->type >= GX_NONE && attrPtr->type <= GX_INDEX16,
      "GDSetVtxDescv: invalid type");

    ASSERTMSGLINE(98,
      attrPtr->attr >= GX_VA_PNMTXIDX && attrPtr->attr <= GX_VA_TEX7MTXIDX ?
    ...
    case GX_VA_TEX0MTXIDX:
      txMtxIdxMask
          = (txMtxIdxMask & ~0x1) | (attrPtr->type << 0);
      break;
    case GX_VA_TEX1MTXIDX:
      txMtxIdxMask
          = (txMtxIdxMask & ~0x2) | (attrPtr->type << 1);
      break;
    case GX_VA_TEX2MTXIDX:
      txMtxIdxMask
          = (txMtxIdxMask & ~0x4) | (attrPtr->type << 2);
      break;
    case GX_VA_TEX3MTXIDX:
      txMtxIdxMask
          = (txMtxIdxMask & ~0x8) | (attrPtr->type << 3);
      break;
    case GX_VA_TEX4MTXIDX:
      txMtxIdxMask
          = (txMtxIdxMask & ~0x10) | (attrPtr->type << 4);
      break;
    case GX_VA_TEX5MTXIDX:
      txMtxIdxMask
          = (txMtxIdxMask & ~0x20) | (attrPtr->type << 5);
      break;
    case GX_VA_TEX6MTXIDX:
      txMtxIdxMask
          = (txMtxIdxMask & ~0x40) | (attrPtr->type << 6);
      break;
    case GX_VA_TEX7MTXIDX:
      txMtxIdxMask
          = (txMtxIdxMask & ~0x80) | (attrPtr->type << 7);
      break;
    case GX_VA_POS:
      posn = attrPtr->type;
      break;
                      ...
                      GXTexGenSrc src_param, u8 normalize, u32 postmtx)
{
  ...
  ASSERTMSGLINE(429, dst_coord < GX_MAX_TEXCOORD,
  ...
  case GX_TG_TEXCOORD6:
    ...
    break;
  default:
    ASSERTMSGLINE(454, 0, "GDSetTexCoordGen: invalid texgen source");
    break;
  }
  ...
  switch (func)
  {
    ...
    case GX_TG_BUMP7:
      ASSERTMSGLINE(478,
        src_param >= GX_TG_TEXCOORD0 && src_param <= GX_TG_TEXCOORD6,
    ...
    default:
      ASSERTMSGLINE(492, 0, "GDSetTexCoordGen: invalid texgen function");
      break;
  }

  GDWriteXFCmd(dst_coord + XF_REG_TEX0_ID,
    XF_REG_TEX(proj, form, tgType, row, embossRow, embossLit));
  GDWriteXFCmd(dst_coord + XF_REG_DUALTEX0_ID,
...
{
  GDWriteBPCmd(BP_LP_SIZE(lineWidth, pointSize, lineOffset,
    pointOffset, lineHalfAspect, 0x22));
}
